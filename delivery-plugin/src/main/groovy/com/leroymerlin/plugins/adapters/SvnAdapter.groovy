...
class SvnAdapter extends BaseScmAdapter {
    ...
    class SvnConfig {
        String username = System.properties["scmUser"]
        String password = System.properties["scmPassword"]
    }
    ...
    void init() {
        ...
        if (username) {
            extension.svnConfig.username = username
        }

        String password = findProperty('release.svn.password')
        if (password) {
            extension.svnConfig.password = password
        }
    ...
    private String svnExec(
        ...
        if (extension.svnConfig.username) {
            if (extension.svnConfig.password) {
                commands.addAll(0, ['--password', extension.svnConfig.password]);
            }
            commands.addAll(0, ['--non-interactive', '--no-auth-cache', '--username', extension.svnConfig.username]);
        }
        commands.add(0, 'svn');
